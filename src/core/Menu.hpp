...
class Menu {
public:
    Menu(TTF_Font* font, SDL_Texture* logo, SDL_Texture* backgroundTexture, SDL_Texture* markerTexture);
...
private:
    ...
    float frameTimer;
    float markerFrameTimer;

    float frameDuration;
    float markerFrameDuration;

    int frameWidth;
    int frameHeight;

    int markerFrameWidth;
    int markerFrameHeight;

    int menuCycleCounter;
};
